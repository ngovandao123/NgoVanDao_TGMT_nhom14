...
class Player(pygame.sprite.Sprite):
	def __init__(self, pos, groups, collision_sprites, semi_collision_sprites, frames, data, attack_sound, jump_sound, input_controller = None):
		# general setup
		...
		self.coyote_time = 160
		self.last_floor_time = pygame.time.get_ticks()
	...
	def input(self):
		...
		if not self.timers['wall jump'].active:
			...
			if keys[pygame.K_x] or gesture and gesture.pressed('attack'):
				self.attack()

			self.direction.x = input_vector.normalize().x if input_vector else input_vector.x

		if keys[pygame.K_SPACE] or gesture and gesture.pressed('jump'):
			self.jump = True		
	...
	def move(self, dt):
		...
		can_ground_jump = self.on_surface['floor'] or current_time - self.last_floor_time <= self.coyote_time
